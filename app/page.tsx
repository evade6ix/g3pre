...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#0b1220] text-slate-100">
      ...
      {selectedProduct && batchMethod && <BatchOrderWorkspace productTitle={selectedProduct.title} method={batchMethod} orders={batchOrders} allOrders={preorderOrders} productIds={preorderIds} onBack={() => { setBatchMethod(null); void reloadProductOrders(selectedProduct.id); }} onUpdated={refresh} onProcessed={markProcessed} />}
      {customerDrawer && <CustomerOrdersDrawer identity={customerDrawer} orders={preorderOrders} productIds={preorderIds} onClose={() => setCustomerDrawer(null)} />}
