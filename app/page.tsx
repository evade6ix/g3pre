...
import CustomerOrdersDrawer, { customerPreorders, type CustomerIdentity } from "./CustomerOrdersDrawer";
...
type Order = { id: string; legacyResourceId: string; adminUrl: string; name: string; createdAt: string; shippingMethod: string; fulfillmentMethod: "pickup" | "shipping"; customerId?: string | null; customerName?: string | null; customerEmail?: string | null; shippingAddress?: { address1: string | null; zip: string | null } | null; items: Item[] };
...
type ProductOrder = { orderId: string; orderName: string; date: string; quantity: number; fulfillmentMethod: string; shippingMethodLabel: string | null; customerId?: string | null; customerName?: string | null; customerEmail?: string | null; shippingAddress?: { name: string | null; address1: string | null; address2: string | null; city: string | null; province: string | null; zip: string | null; country: string | null } | null; items: Item[] };
...
export default function HomePage() {
  ...
  const [customerDrawer, setCustomerDrawer] = useState<CustomerIdentity | null>(null);
  ...
  const customerCount = (identity: CustomerIdentity) => customerPreorders(identity, preorderOrders, preorderIds).length;
    ...
    <main className="min-h-screen bg-[#0b1220] text-slate-100">
      ...
      {selectedProduct && <div className="fixed inset-0 z-40 flex justify-end bg-black/65" role="dialog" aria-modal="true" aria-label={`Orders for ${selectedProduct.title}`}>
        ...
        <aside className="h-full w-full max-w-[680px] overflow-y-auto border-l border-white/10 bg-[#111a28] p-6 shadow-2xl md:p-8">
          ...
          <div className="mt-8"><h3 className="mb-4 font-semibold">Customer orders</h3>{productLoading ? <p className="text-sm text-slate-400">Loading orders…</p> : productOrders.filter((order) => !processedOrderIds.has(order.orderId)).length ? productOrders.filter((order) => !processedOrderIds.has(order.orderId)).map((order) => <div key={order.orderId} className="mb-3 rounded-xl border border-white/10 bg-white/[.04] p-4"><button onClick={() => { const match = orders.find((item) => item.legacyResourceId === order.orderId); if (match) void openOrder(match, true); }} className="flex w-full items-center justify-between text-left"><span><span className="font-semibold text-white">{order.orderName}</span><span className="mt-1 block text-xs text-slate-400">{order.customerName || "Customer"} · {formatDate(order.date)} · {order.fulfillmentMethod === "pickup" ? "Pickup" : "Shipping"}</span></span><span className="text-sm font-semibold text-cyan-300">× {order.quantity} →</span></button><CustomerPreorderButton count={customerCount(order)} onClick={() => setCustomerDrawer(order)} /></div>) : <Empty text="No unfulfilled orders for this product." />}</div>
        </aside>
      </div>}
      {selectedProduct && batchMethod && <BatchOrderWorkspace productTitle={selectedProduct.title} method={batchMethod} orders={batchOrders} allOrders={preorderOrders} productIds={preorderIds} processedOrderIds={processedOrderIds} onBack={() => { setBatchMethod(null); void reloadProductOrders(selectedProduct.id); }} onUpdated={refresh} onProcessed={markProcessed} />}
      {customerDrawer && <CustomerOrdersDrawer identity={customerDrawer} orders={preorderOrders} productIds={preorderIds} onProcessed={markProcessed} onUpdated={refresh} onClose={() => setCustomerDrawer(null)} />}

    </main>
  );
}

function CustomerPreorderButton({ count, onClick }: { count: number; onClick: () => void }) { return <button onClick={onClick} className={`mt-3 rounded-lg px-3 py-2 text-xs font-semibold ${count > 1 ? "border border-amber-300/30 bg-amber-300/10 text-amber-200 hover:bg-amber-300/20" : "border border-white/10 text-slate-400 hover:bg-white/10"}`}>{count > 1 ? `${count} active preorders · View all →` : "Only open preorder · View →"}</button>; }
